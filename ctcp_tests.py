...
def bidirectional():
  ...
  if sent_str != test_str_send or recv_str != test_str_recv:
    ...
    # print("sent_str: {0} | test_str_send: {1}".format(sent_str, test_str_send))
    # print("recv_str: {0} | test_str_recv: {1}".format(recv_str, test_str_recv))
    return False
...
def fin_sent():
  ...
  if not read_segments_from(client):
    print("Failed in first write some data")
    return False
  time.sleep(1)
  ...
  segments = read_segments_from(client)
  if not segments:
    print("No segment when read EOF")
    return False
  if "FIN" not in segments[0].flags:
    print("segment sent from client is NOT a FIN")
  return True
...
def connection_teardown():
  ...
  messages_from_client = read_debug_messages_from(client)
  messages_from_server = read_debug_messages_from(server)
  # print("messages_from_client: {0}".format(messages_from_client))
  # print("messages_from_server: {0}".format(messages_from_server))
  return (
    DEBUG_TEARDOWN in messages_from_client and
    DEBUG_TEARDOWN in messages_from_server
  )
...
def multiple_clients():
  ...
  for i, client in enumerate(clients):
    ...
    time.sleep(1)
    res = read_from(server)
    if not res == test_strs[i]:
		# print("read_from_server: {0}".format(res))
		# print("test_strs[{0}]: {1}".format(i,test_strs[i]))
		return False
  return True
